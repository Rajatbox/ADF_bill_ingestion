...
Purpose: Automatically populate and maintain reference/lookup tables by
         discovering new values from processed Landmark billing data.

         Block 0: Auto-discover integrated carriers (CarrierName column) into dbo.carrier
                  e.g. "DHL eCommerce", "Landmark Global"
         Block 1: Sync shipping methods from ServiceName column, keyed by
...
BEGIN TRY
    ...
    ================================================================================
    Landmark is an aggregator — CarrierName holds the actual fulfillment carrier
    (e.g., "DHL eCommerce", "Landmark Global"). Insert any new carrier names into
    ...
    */
    ...
    WHERE cb.file_id = @File_id
      ...
      AND NOT EXISTS (
            ...
            WHERE LOWER(c.carrier_name) = LOWER(l.carrier_name)
      );
    ...
    Key:  (carrier_id = Landmark carrier_id, method_name = ServiceName,
           integrated_carrier_id = resolved from CarrierName)

    ServiceName examples: "DHL SmartMail Parcel Plus Expedited",
    ...
    */
    ...
    WHERE cb.file_id = @File_id
      AND NULLIF(TRIM(l.service_name), '') IS NOT NULL
      AND NOT EXISTS (
            SELECT 1
            FROM dbo.shipping_method sm
            WHERE sm.carrier_id = @Carrier_id
              AND sm.method_name = l.service_name
              AND (
    ...
    */
    ...
    SELECT DISTINCT
        ...
        l.charge_name,
        CASE
            WHEN LOWER(l.charge_name) = 'freight charge' THEN 1
            ELSE 0
        END AS is_freight,
    ...
    WHERE cb.file_id = @File_id
      ...
      AND NOT EXISTS (
            ...
            WHERE ct.charge_name = l.charge_name
...
Aggregator rule: Block 0 auto-discovers integrated carriers before Block 1
